...
export const BeadingFrame: FC<{
    ...
    onContextMenu,
        ...
        const { selectedColumn, selectedRow, setSelectedColumn, setSelectedRow } = useGridSelection();

        const { height: cellHeight, width: cellWidth } = getGridCellRenderSize(options, styles);
        ...
        const columnsTextArray: Array<TextState> = Array.from({ length: width }, (_, index) => ({ gridIndex: index, gridName: "all", patternIndex: index }));
        const rowTextArray: Array<TextState> = Array.from({ length: height }, (_, index) => ({ gridIndex: index, gridName: "all", patternIndex: index }));
